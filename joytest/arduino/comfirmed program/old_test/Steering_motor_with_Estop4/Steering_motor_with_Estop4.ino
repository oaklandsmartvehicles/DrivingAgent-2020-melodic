/*      NEED CHECK STEERINGMOTOR ROTATION DIRECTION and POTENTIOMETER ROTATION DIRECTION
...
void loop() {
  ...
  if (estop_flag == true) {
    ...
    if (rotstart == true) {
      if (rotdir > 0) digitalWrite(CW_plus, true);
      else if (rotdir < 0) digitalWrite(CW_plus, false);
      rotstart = false;
    }
    if (rotcount) {
