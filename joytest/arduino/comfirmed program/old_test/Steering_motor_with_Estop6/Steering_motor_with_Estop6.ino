...
void loop() {
  ...
  if (steering_topic == true) {
    steering_count1 = steering_count;
    steering_dir1 = steering_dir;
    if (steering_dir1 > 0) digitalWrite(CW_plus, true);
    if (steering_dir1 < 0) digitalWrite(CW_plus, false);
    delaytime = 5;
    steering_topic = false;
  }
  if (steering_count1 > 0) {
    ...
    c_steering.data = -(analogRead(steer_angle) - 512.0) / 512.0 * 40;
#else
    c_steering.data += (steering_dir1 / ((400. / 360.) * (360. / (37.5 / 4))));
#endif
